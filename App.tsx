  ...
  return (
    <div className="flex flex-col h-screen bg-[#fdfaff] text-slate-700 overflow-hidden font-inter selection:bg-pink-200 selection:text-pink-900">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {images.length > 0 && (
          <aside className="w-64 bg-white/60 border-r border-purple-100 flex flex-col shrink-0 transition-all z-20 backdrop-blur-sm">
            <div className="p-4 border-b border-purple-100">
              <h2 className="text-xs font-bold text-purple-400 uppercase tracking-widest mb-3 font-[Fredoka]">Your Designs</h2>
              <button 
                onClick={handleBatchDownload}
                disabled={images.length === 0}
                className="w-full flex items-center justify-center gap-2 bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs py-2.5 rounded-xl border border-purple-200 disabled:opacity-50 transition-colors font-bold shadow-sm"
              >
                <Download className="w-3.5 h-3.5" /> Download All JPG
              </button>
            </div>
                ...
                <div 
                  ...
                  className={`group flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-all border ${
                    selectedId === img.id 
                      ...
                      : 'hover:bg-white/80 border-transparent hover:border-purple-100'
                  }`}
                >
                  <div className="w-12 h-12 bg-slate-100 rounded-lg overflow-hidden shrink-0 relative border border-slate-200">
                    <img src={img.originalUrl} className="w-full h-full object-cover" alt="" />
                    {img.isProcessing && (
                       <div className="absolute inset-0 bg-white/60 flex items-center justify-center backdrop-blur-[1px]">
                          <RefreshCcw className="w-5 h-5 animate-spin text-pink-400" />
                       </div>
                    )}
                  </div>
                  ...
                  <button 
                    ...
                    className="text-slate-400 hover:text-red-400 p-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                  >
        ...
        <main className="flex-1 bg-slate-50 relative overflow-hidden flex flex-col items-center">
          {images.length === 0 ? (
            // Empty State
        ...
        {images.length > 0 && (
          <aside className="w-80 bg-white/60 border-l border-purple-100 p-6 flex flex-col shrink-0 overflow-y-auto z-20 backdrop-blur-sm">
            <div className="mb-6">
                       ...
                       {[3, 6, 9].map(count => (
                         <button
                           ...
                           className={`flex-1 text-xs py-1.5 rounded-md transition-all font-bold ${
                             currentSettings.variantCount === count 
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`relative py-3 px-2 text-xs font-medium rounded-xl border-2 transition-all overflow-hidden ${
                      currentSettings.mode === StencilMode.HOLLOW 
                  ...
                  <button
                    ...
                    className={`relative py-3 px-2 text-xs font-medium rounded-xl border-2 transition-all overflow-hidden ${
                      currentSettings.mode === StencilMode.SOLID 
                        ? 'bg-white border-purple-400 text-purple-600 shadow-md shadow-purple-100' 
                        : 'bg-white/50 border-transparent text-slate-400 hover:border-purple-200 hover:bg-white'
                    }`}
                  >
